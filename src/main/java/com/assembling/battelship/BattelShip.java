package com.assembling.battelship;
    ...
    private static boolean hasErrors(int row, int col, int dir, Player p, int count)
    {
        ...
        if (dir == 0) // Hortizontal
        {
            for (int i = col; i < col+length; i++)
            {
                for(int r=row-1; r<=row+1; r++){
                    if(r>=0 && r<10 && i>=0 && i<10){
                        if(p.playerGrid.hasShip(r, i)){
                            System.out.println("SHIP OVERLAPS OR TOUCH EACH OTHERS");
                            return true;
                        }
                    }
                }
        ...
        else if (dir == 1) // Vertical
        {
            ...
            for (int i = row; i < row+length; i++)
            {
                ...
                if(i>=0 && i<10 && c>=0 && c<10){
                    if(p.playerGrid.hasShip(i, c)){
                        System.out.println("SHIP OVERLAPS OR TOUCH EACH OTHERS");
                        return true;
                    }
                }
